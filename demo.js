...
(() => {
  ...
  const validate=()=>{
    ...
    const c=parseFloat(cost.value); if(Number.isNaN(c)||c<0) errs.push("Enter your cost (0 or greater).");
  ...
  function buildStep2(){
    ...
    const conf=[{label:"Category",pct:82},{label:"Era",pct:66},{label:"Material",pct:74},{label:"Condition",pct:95}];
  ...
  function renderProfit(){
    ...
    const gross=state.suggested, fees=+(gross*feeRate+state.feeFixed).toFixed(2), net=+(gross-fees-ship).toFixed(2), profit=+(net-state.cost).toFixed(2);
  ...
  function awardXP(){
    let xp=30; if(state.file) xp+=25; if(state.desc.length>40) xp+=20; if(state.condition) xp+=10; if(state.cost>0) xp+=15;
